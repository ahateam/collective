		...
		<el-row class="row-box1">
			<template>
				<el-table :data="tableData" border style="width: 100%">
					<el-table-column prop="type" label="审批分类" :formatter="typeFilter">
					</el-table-column>
					<el-table-column prop="data" label="被审批相关人" :formatter="userFilter">
					</el-table-column>
					<el-table-column prop="examineDate" label="最近审批时间" :formatter="timeFilter">
					</el-table-column>
					<el-table-column prop="status" label="审批状态" :formatter="statusFilter">
					</el-table-column>
	...
	export default {
		...
		methods: {
			...
			timeFilter(row, col, val) {
				let timer = new Date(val).toLocaleDateString() + ' ' + new Date(val).toLocaleTimeString('chinese', {
					hour12: false
				})
				return timer
			...
			infoBtn(row) {
				...
				this.$store.state.nowType = this.type
				this.$store.state.nowStatus = this.isActive
				if (row.type == 1) {
					this.$router.push({
			...
			activeBtn(active) {
				this.$store.state.nowStatus = active
				this.page = 1
		...
		mounted() {
			...
			this.isActive = this.$store.state.nowStatus
			this.type = this.$store.state.nowType
			let cnt = {
				districtId: localStorage.getItem('orgId'), // Long 组织编号
				type: this.type, // Byte 类型
				status: this.isActive, // Byte 状态
