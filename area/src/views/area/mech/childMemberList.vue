    ...
    <el-row :gutter="40">
      <el-col :span="5" style="overflow: hidden">
        <template>
          <el-col :span="24">
            <div
              style="width:auto;height: 30px;padding: 1rem;border-left: 4px solid #63a35c;font-size: 1.6rem; "
            >全部职务</div>
          </el-col>
          <el-col :span="24">
            <div
              :class=" item.name == roleActive.name?'post-list post-active':'post-list'"
              v-for="(item,index) in newRoleList"
              :key="index"
              @click="roleChange(item,index)"
            >
...
export default {
  ...
  data() {
    return {
      ...
      //职务角色列表
      newRoleList: [
        { name: "外部成员", orgId: 100, roleId: 1 },
        { name: "组织成员", orgId: 100, roleId: 2 }
      ], //角色列表
      roleList: [], //角色列表
      roleActive: "", //选中的角色
