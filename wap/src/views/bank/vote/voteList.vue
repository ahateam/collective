<template>
    <div>
        <bank-head title="投票列表"></bank-head>
        <div class="main-box">
            <van-list
                    v-model="loading"
                    :finished="finished"
                    finished-text="没有更多投票信息了"
                    @load="onLoad"
            >
                <div v-if="voteList.length>0">
                    <div v-for="(item,index) in voteList" :key="index">
                        <div class="vote-item-box" @click="showBtn(item)">
...
<script>
    ...
    export default {
        ...
        mounted() {
            if(this.$route.params.orgId == '' ||this.$route.params.orgId == undefined){
                this.$router.push('/bankHome')
            }
