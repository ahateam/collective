  ...
  <div>
    ...
    <el-row class="row-box1">
      <el-col :span="24">
        ...
        <el-button type="primary" @click="createAssetBtn()">单资产录入</el-button>
      </el-col>
      ...
      <el-col :span="24" style="margin-top: 15px">
        <template>
          <el-table :data="tableData" border style="width: 100%">
            ...
            <el-table-column prop="amount" label="总数据量"></el-table-column>
            <el-table-column prop="createTime" label="创建时间" :formatter="timeFilter"></el-table-column>
            <el-table-column label="导入进度">
              <template slot-scope="scope">
                <span v-if="scope.row.amount == 0">0</span>
                <span v-else>{{((scope.row.successCount /scope.row.amount)*100).toFixed(2)}}%</span>
              </template>
            </el-table-column>
            <el-table-column prop="status" label="任务状态" :formatter="statusFilter"></el-table-column>
            <el-table-column fixed="right" label="操作">
              <template slot-scope="scope">
                ...
                >导入数据表</el-button>
                <el-button
                  @click="infoResBtn(scope.row)"
                  type="text"
                  size="small"
                  v-if="scope.row.status == '3' "
                >查看任务报告</el-button>
                <el-button
                  @click="infoErrorBtn(scope.row)"
                  type="text"
                  size="small"
                  v-if="scope.row.status == '4' "
                >
                  <span style="color: #f44;">查看失败数据</span>
                </el-button>
  ...
  methods: {
    ...
    timeFilter(row, col, val) {
      ...
      return (
        ...
        " " +
        timer.toLocaleTimeString("chinese", { hour12: false })
      );
    },
